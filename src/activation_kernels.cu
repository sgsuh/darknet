...
__device__ float prelu_activate_kernel(float x, float p){return (x>0) ? x : p*x;}
...
__device__ float prelu_gradient_kernel(float x, float p){return (x>0) ? 1 : p;}
...
__global__ void activate_array_prelu_kernel(float *x, int n, float p)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < n) x[i] = prelu_activate_kernel(x[i], p);
}

__global__ void gradient_array_prelu_kernel(float *x, int n, float p, float *delta)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < n) delta[i] *= prelu_gradient_kernel(x[i], p);
}

extern "C" void activate_array_prelu_gpu(float *x, int n, float p)
{
    activate_array_prelu_kernel<<<cuda_gridsize(n), BLOCK>>>(x, n, p);
    check_error(cudaPeekAtLastError());
}

extern "C" void gradient_array_prelu_gpu(float *x, int n, float p, float *delta)
{
    gradient_array_prelu_kernel<<<cuda_gridsize(n), BLOCK>>>(x, n, p, delta);
    check_error(cudaPeekAtLastError());
}
